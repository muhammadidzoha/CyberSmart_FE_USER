...
import { umurChart } from "./variabels/chart";
import { Card, Typography } from "@material-tailwind/react";

Highcharts3D(Highcharts);

const TABLE_ROWS = umurChart.series[0].data.map((dataPoint) => ({
  name: dataPoint.name,
  y: dataPoint.y,
...
const Umur = () => {
  ...
  const totalPerempuan = TABLE_ROWS.reduce(
    (acc, row) => acc + row.perempuan,
    0
  );
  const totalJiwa = totalLaki + totalPerempuan;

  const persentaseTotalLaki =
    totalLaki !== 0 ? (totalLaki / totalJiwa) * 100 : 0;
  ...
  return (
    <div>
      <HighchartsReact highcharts={Highcharts} options={umurChart} />
      ...
      <Card color="transparent" shadow={false}>
        <table className="w-full min-w-max table-auto text-left">
          ...
          <tbody>
            {TABLE_ROWS.map(({ name, y, laki, perempuan }, index) => {
